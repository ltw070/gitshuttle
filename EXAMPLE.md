...
현재 브랜치 전체가 아니라 선택한 최신 tip만 직접 제어하려면 `--recent 2 --bundle-scope full`처럼 사용할 수 있습니다.
`--full-branch`와 `--bundle-scope full`은 선택한 tip까지 전체 이력을 담기 때문에 일반 부분 bundle보다 커질 수 있지만, prerequisite 실패를 피할 수 있습니다.

GitShuttle은 bundle 이력 이전에 집중합니다. 대상 repo에 기존 코드가 있으면 bundle을 migration 브랜치에 import하고, 검토 후 Git merge로 합치는 방식으로 운영하세요.
